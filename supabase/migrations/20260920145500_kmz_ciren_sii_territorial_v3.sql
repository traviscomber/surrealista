...
where coalesce(k.is_active, true)
  ...
  and not exists (
    ...
    where e.kmz_id = k.id
      ...
      and coalesce(e.metadata->>'pipeline','') = 'kmz-ciren-backfill-v3'
      and (
...
comment on view public.kmz_ciren_enrichment_queue is
  'Server-only queue for KMZ CIREN v3 enrichment using exact ROL, SII-resolved commune and spatial evidence. Canonical KMZ rows remain unchanged.';

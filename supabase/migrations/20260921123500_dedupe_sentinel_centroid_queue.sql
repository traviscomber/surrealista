...
comment on view public.kmz_sentinel_centroid_queue is
  'Server-only queue of unique Sentinel centroid targets. Deduplicates exact ROL + SII point fingerprints, reuses recent canonical Sentinel memory, and preserves provider retry semantics without treating unavailable data as a negative property fact.';

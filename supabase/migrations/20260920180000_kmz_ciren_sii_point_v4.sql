drop view if exists public.kmz_ciren_enrichment_queue;

create view public.kmz_ciren_enrichment_queue
...
where coalesce(k.is_active, true)
  ...
  and not exists (
    ...
    where e.kmz_id = k.id
      ...
      and coalesce(e.metadata->>'pipeline','') = 'kmz-ciren-backfill-v4'
      and (
...
comment on view public.kmz_ciren_enrichment_queue is
  'Server-only queue for KMZ CIREN v4 enrichment using exact ROL, canonical region, SII-resolved commune, SII point and KMZ geometry. Canonical KMZ rows remain unchanged.';
...
async function runMaintenance() {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return { deduplication: "skipped: missing SUPABASE config", aggregation: "skipped: missing SUPABASE config" }
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const summary = { durationMs: Date.now() - startedAt, totals, maintenance, failedSources: failedSources.map((item) => item.source), quarantinedSources: QUARANTINED_SOURCES }

    if (success) console.log(requestId, "[v0] Nightly property scraper cron finished", summary)
    else console.error(requestId, "[v0] Nightly property scraper cron finished with errors", summary)

    return NextResponse.json(
      { success, partial: !success, timestamp: new Date().toISOString(), durationMs: Date.now() - startedAt, totals, maintenance, maintenanceErrors, failedSources, sources, quarantinedSources: QUARANTINED_SOURCES },

  ...
  try {
    ...
    for (const kmz of kmzFiles) {
      try {
        ...
        const topOwner = ownerCandidates.length > 0 ? ownerCandidates[0] : null
        const ownerSummary = topOwner || { name: null, confidence: 0, type: "unknown" }
        ...
        const unifiedResult = {
          ...
          owner_candidate: ownerSummary.name,
          owner_confidence: ownerSummary.confidence,
          owner_type: ownerSummary.type,
        ...
        if (!dry_run && topOwner) {
          const updatedMetadata = {
            ...
            public_owner_candidate: topOwner.name
              ? {
                  name: topOwner.name,
                  type: topOwner.type === "company" ? "company" : "person",
                  confidence: topOwner.confidence,
                  reason: `Extracted from KMZ ${topOwner.source} using ${topOwner.pattern}`,
                  source: `kmz_${topOwner.source}`,
                  dateFound: now,
                }
              : null,
            owner_confidence: topOwner.confidence,
            owner_type: topOwner.type,
            owner_research_leads: ownerCandidates.slice(0, 5).map((candidate) => ({
              name: candidate.name,
      ...
      for (const update of updates) {
        try {
          const { error, data } = await supabase
            .from("kmz_collection")
            .update({ owner: update.owner, metadata: update.metadata })
            .eq("id", update.id)
            .select()

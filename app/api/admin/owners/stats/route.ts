...
export async function GET() {
  try {
    ...
    const stats = {
      total: owners.length,
      high_confidence: owners.filter((item: any) => parseFloat(item.metadata?.web_owner_confidence || 0) >= 0.85).length,
      ...
      low_confidence: owners.filter((item: any) => parseFloat(item.metadata?.web_owner_confidence || 0) < 0.7).length,

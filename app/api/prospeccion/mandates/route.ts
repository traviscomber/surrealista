...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select("id,name,client_id,region,commune,species,min_ha,max_ha,status,last_candidate_count,last_new_candidate_count,last_run_at,created_at")
...
export async function POST(request: Request) {
  ...
  if (clientId) {
    ...
    if (!["buyer", "investor", "both"].includes(client.client_type)) {
      return NextResponse.json({ error: "El mandato debe vincularse a un cliente comprador o inversionista." }, { status: 409 })
    }
    if (client.status === "archived") return NextResponse.json({ error: "El cliente está archivado." }, { status: 409 })
  }

  const { data, error } = await supabase
    .from("prospecting_mandates")
    .insert({ name, client_id: clientId, region, commune, species, min_ha: minHa, max_ha: maxHa, created_by: "internal-operator" })
    .select("id,name,client_id,region,commune,species,min_ha,max_ha,status,last_candidate_count,last_new_candidate_count,last_run_at,created_at")
    .single()

...
export default function CotizadorPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    features: '',
    additional_info: '',
    macrofiltros: emptyMacrofilters,
            ...
            <CardContent>
              {loading ? (
                <div className="flex min-h-72 flex-col items-center justify-center gap-3 text-center text-sm text-muted-foreground">
                  ...
                  <p>Procesando los antecedentes ingresados.</p>
                </div>
              ) : result ? (
                <div className="space-y-5">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold">Metodología informada</h3>
                    <p className="mt-2 text-sm leading-6 text-muted-foreground">{result.methodology || 'El servicio no informó una metodología.'}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold">Análisis de comparables</h3>
                    <p className="mt-2 text-sm leading-6 text-muted-foreground">{result.comparable_analysis || 'No se informó análisis de comparables.'}</p>
                  </div>
                  {result.market_factors?.length ? (
                    <div><h3 className="text-sm font-semibold">Factores considerados</h3><ul className="mt-2 space-y-2 text-sm text-muted-foreground">{result.market_factors.map((item) => <li key={item} className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{item}</li>)}</ul></div>
                  ) : null}
                  {result.data_sources?.length ? (
                    <div><h3 className="text-sm font-semibold">Fuentes declaradas</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{result.data_sources.join(', ')}</p></div>
                  ) : null}
                </div>
              ) : (
                <div className="flex min-h-72 flex-col items-center justify-center gap-3 text-center">
                  <BarChart3 className="h-7 w-7 text-muted-foreground" />
                  <div><p className="font-medium">Aún no hay una referencia calculada</p><p className="mt-1 max-w-xs text-sm leading-6 text-muted-foreground">Completa los antecedentes mínimos y ejecuta el cálculo para revisar el rango y sus fundamentos.</p></div>
                </div>
              )}
            </CardContent>
          </Card>

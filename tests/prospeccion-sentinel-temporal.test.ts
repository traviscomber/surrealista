...
test("uses the CIREN polygon and derives an interannual Sentinel comparison", async () => {
  ...
  try {
    ...
    assert.equal(statisticalRequest.aggregation.aggregationInterval.of, "P1M")
    assert.equal(new Date(statisticalRequest.aggregation.timeRange.from).getUTCDate(), 1)
    assert.equal(new Date(statisticalRequest.aggregation.timeRange.to).getUTCDate(), 1)
    assert.equal(new Date(statisticalRequest.aggregation.timeRange.from).getUTCHours(), 0)
    assert.equal(new Date(statisticalRequest.aggregation.timeRange.to).getUTCHours(), 0)

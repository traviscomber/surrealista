...
import { Search, FileText, Users, MapPin, MessageSquare, CheckSquare, Folder, Mail, Phone, Calendar, Building2, Map } from 'lucide-react'
...
export function GlobalCommandPalette() {
  ...
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      const { data: clients } = await supabase
        ...
        .or(`first_name.ilike.%${searchQuery}%,last_name.ilike.%${searchQuery}%,email.ilike.%${searchQuery}%,company_name.ilike.%${searchQuery}%,main_interest.ilike.%${searchQuery}%`)
        .limit(10)
      ...
      if (clients) {
        allResults.push(
          ...clients.map((client) => ({
            ...
            title: `${client.first_name || ''} ${client.last_name || ''}`.trim() || client.company_name || 'Cliente sin nombre',
      ...
      if (campos) {
        allResults.push(
          ...campos.map((campo) => ({
            ...
            description: campo.description || `${campo.placemarks_count || 0} puntos`,
      ...
      const { data: messages } = await supabase
        ...
        .or(`name.ilike.%${searchQuery}%,email.ilike.%${searchQuery}%,subject.ilike.%${searchQuery}%,message.ilike.%${searchQuery}%`)
        .limit(10)
  ...
  const getStatusColor = (status?: string) => {
    ...
    if (statusLower.includes('hot') || statusLower.includes('activ') || statusLower.includes('vigente')) return 'destructive'

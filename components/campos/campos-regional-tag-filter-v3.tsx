...
const VALID_TAGS = new Set(TAG_GROUPS.flatMap(g=>g.options.map(o=>o.tag)))
...
export function CAMPOSRegionalTagFilterV3(){
  ...
  useEffect(()=>{
    setSelectedRegions(splitParam(searchParams.get("regions")))
    setSelectedTags(splitParam(searchParams.get("tags")).filter(tag=>VALID_TAGS.has(tag)))
  ...
  useEffect(()=>{const onKey=(event:KeyboardEvent)=>{if(event.key==="Escape")setOpen(false);if(event.key==="/"&&!event.metaKey&&!event.ctrlKey&&!event.altKey){const target=event.target as HTMLElement|null;if(target?.tagName==="INPUT"||target?.tagName==="TEXTAREA")return;event.preventDefault();setOpen(true);requestAnimationFrame(()=>searchRef.current?.focus())}};window.addEventListener("keydown",onKey);return()=>window.removeEventListener("keydown",onKey)},[])
  ...
  useEffect(()=>{if(!hydratedRef.current)return;const params=new URLSearchParams(searchParams.toString());if(selectedRegions.length)params.set("regions",selectedRegions.join("|"));else params.delete("regions");if(selectedTags.length)params.set("tags",selectedTags.join("|"));else params.delete("tags");if(query.trim())params.set("q",query.trim());else params.delete("q");router.replace(`${pathname}?${params.toString()}`,{scroll:false})},[pathname,query,router,selectedRegions,selectedTags])
  ...
  const openField=(id:string)=>{const params=new URLSearchParams(searchParams.toString());params.set("kmz",id);if(selectedRegions.length)params.set("regions",selectedRegions.join("|"));if(selectedTags.length)params.set("tags",selectedTags.join("|"));if(query.trim())params.set("q",query.trim());setOpen(false);router.push(`${pathname}?${params.toString()}`,{scroll:false})}

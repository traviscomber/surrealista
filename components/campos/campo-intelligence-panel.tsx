...
export function CampoIntelligencePanel({ record, ciren }: { record: KmzInventoryRecord; ciren: CirenSignal }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      supabase
        .from("kmz_collection")
        .select("pic,pic_phone,pic_email,updated_at")
        .eq("id", record.id)
        .maybeSingle(),
    ]).then(([nearbyResult, marketResult, publicResult, contactResult]) => {
      if (cancelled) return
      if (nearbyResult.error || marketResult.error || publicResult.error || contactResult.error) {
        console.warn("[CAMPOS] intelligence evidence partially unavailable", {
          nearby: nearbyResult.error,
          market: marketResult.error,
          public: publicResult.error,
          contact: contactResult.error,
        })
        setFailed(true)
      }
      setNearby((nearbyResult.data || []) as NearbyFeature[])
      setComparables((marketResult.data || []) as MarketComparable[])
      setPublicMetrics((publicResult.data || []) as PublicMetric[])
      setContact((contactResult.data || null) as ContactRow | null)
      setLoading(false)
    }).catch((error) => {
      if (cancelled) return
  ...
  }, [record.id, record.region, supabase])
...
function Evidence({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {

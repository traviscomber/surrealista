...
export function MarketGeocodeHealth() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [active, geocoded, neverAttempted, noMatch, errors, recent, retryReady] = await Promise.all([
      ...
      const firstError = [active, geocoded, neverAttempted, noMatch, errors, recent, retryReady].find((result) => result.error)?.error
    ...
    <Card>
        ...
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">
              Estado real del proceso que georreferencia avisos externos para habilitar distancias, vecinos, cruces KMZ y scoring territorial.
            </p>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Metric label="Georreferenciados" value={loading ? "…" : health.geocoded.toLocaleString("es-CL")} detail={`${coverage.toFixed(1)}% de cobertura`} />
          <Metric label="Sin intentar" value={loading ? "…" : health.neverAttempted.toLocaleString("es-CL")} detail="Prioridad de las próximas corridas" />
          <Metric label="Intentos última hora" value={loading ? "…" : health.attemptedLastHour.toLocaleString("es-CL")} detail="Ritmo observado, no estimado" />
        </div>

        <div className="grid gap-3 sm:grid-cols-3">
          <div className="flex items-center gap-3 rounded-lg border px-4 py-3">
            <Clock3 className="h-4 w-4 text-muted-foreground" />
            <div><div className="text-sm font-medium">{health.noMatch.toLocaleString("es-CL")} sin coincidencia</div><div className="text-xs text-muted-foreground">Reintento con backoff automático</div></div>
          </div>
          <div className="flex items-center gap-3 rounded-lg border px-4 py-3">
            <RotateCcw className="h-4 w-4 text-muted-foreground" />
            <div><div className="text-sm font-medium">{health.retryReady.toLocaleString("es-CL")} listos para retry</div><div className="text-xs text-muted-foreground">Ya cumplieron su espera</div></div>
          </div>
          <div className="flex items-center gap-3 rounded-lg border px-4 py-3">
            {health.errors > 0 ? <AlertTriangle className="h-4 w-4 text-amber-500" /> : <CheckCircle2 className="h-4 w-4 text-emerald-500" />}
            <div><div className="text-sm font-medium">{health.errors.toLocaleString("es-CL")} errores</div><div className="text-xs text-muted-foreground">Fallos transitorios pendientes</div></div>
          </div>
        </div>

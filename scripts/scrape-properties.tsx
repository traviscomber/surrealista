...
export default function ScrapeProperties() {
  ...
  const scrapeProperties = async () => {
    ...
    for (let i = 0; i < urls.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(async (url) => {
          try {
            // In a real implementation, this would use a proxy or server-side function
            // to fetch and parse the HTML from each URL
            ...
            const property: PropertyData = {
              ...
              price: Math.floor(Math.random() * 500000000) + 100000000, // Random price between 100M and 600M
  ...
  // Helper functions for generating random property data
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Scrape Properties from Sur-Realista</CardTitle>
      </CardHeader>
      <CardContent>

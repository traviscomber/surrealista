...
const SOUTHERN_COMMUNES = [
  { code: '10101', expectedName: 'Puerto Montt' },
  { code: '10109', expectedName: 'Puerto Varas' },
  { code: '10301', expectedName: 'Osorno' },
  { code: '10201', expectedName: 'Castro' },
  { code: '14101', expectedName: 'Valdivia' },
  { code: '11101', expectedName: 'Coyhaique' },
] as const
...
function metricFactory(payload: JsonRecord, endpoint: string, scrapedAt: string) {
  ...
  return (dataset: string, metric: string, value: unknown, unit: string | null, period: string | null = null, dimensions: Record<string, unknown> = {}): Metric | null => {
    const numeric = asNumber(value)
    if (numeric === null) return null
    const dimensionKey = Object.entries(dimensions).sort(([a], [b]) => a.localeCompare(b)).map(([key, val]) => `${key}:${String(val)}`).join('|')
    return {
      ...common,
      dataset,
      metric,
...
function extractMetrics(payload: JsonRecord, endpoint: string): Metric[] {
  ...
  metrics.push(
    ...
    make('demographics_housing', 'housing_occupation_rate_pct', asNumber(housing.occupationRate) == null ? null : Number(housing.occupationRate) * 100, 'percent'),
  ...
  for (const [ageBand, value] of Object.entries(population)) {
    if (!ageBand.startsWith('age')) continue
  ...
  const multifamily = markets.multifamily || {}
  metrics.push(
    make('market_multifamily', 'multifamily_buildings_total', multifamily.totalEdificios, 'count'),
    make('market_multifamily', 'multifamily_units_total', multifamily.unidadesTotales, 'count'),
    make('market_multifamily', 'multifamily_vacancy_rate_pct', multifamily.vacanciaPromedio, 'percent'),
  )

  const offices = markets.oficinas || {}
  metrics.push(
    make('market_offices', 'office_buildings_total', offices.totalEdificios, 'count'),
    make('market_offices', 'office_surface_total_m2', offices.superficieTotal, 'm2'),
    make('market_offices', 'office_vacancy_rate_pct', offices.vacanciaPromedio, 'percent'),
  )
...
async function persistMetrics(metrics: Metric[]) {
  ...
  for (let index = 0; index < metrics.length; index += 100) {
    ...
    const { data: existing, error: lookupError } = await supabase.from('market_public_metrics').select('fingerprint').in('fingerprint', keys)
    if (lookupError) {
    ...
    const { data, error } = await supabase.from('market_public_metrics').upsert(chunk, { onConflict: 'fingerprint', ignoreDuplicates: false }).select('fingerprint')
    if (error) {
...
export async function scrapeIncitiDataHub(options: { persist?: boolean; communeCodes?: string[] } = {}): Promise<IncitiDataHubResult> {
  ...
  for (const target of requested) {
    ...
    try {
      ...
      if (actualName !== target.expectedName || actualCode !== target.code) {
  ...
  const saved = persist ? await persistMetrics(allMetrics) : { inserted: 0, updated: 0, skipped: 0, errors: [] as string[] }
